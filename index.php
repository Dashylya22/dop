...
if (empty($userService->getAllUsers())) {
    ...
    showMessage("Создан тестовый пользователь: admin@test.com / admin123");
}

// Главное меню
function showMainMenu($userService, $projectService, $taskService) {
...
function showLoginForm($userService) {
    echo "\n=== ВХОД В СИСТЕМУ ===\n";
    echo "Email: ";
    ...
    if ($user) {
        showMessage("Успешный вход! Добро пожаловать, " . $user->email);
    } else {
        showMessage("Ошибка входа! Проверьте email и пароль.");
    }
}

function showRegisterForm($userService) {
    echo "\n=== РЕГИСТРАЦИЯ ===\n";
    echo "Email: ";
    $email = trim(fgets(STDIN));
    echo "Пароль: ";
    ...
    if ($user) {
        ...
        showMessage("Ошибка регистрации! Пользователь с таким email уже существует.");
    }
}

function showProjectMenu($projectService, $taskService) {
    while (true) {
        ...
        switch ($choice) {
            case '1':
                ...
                if ($project) {
                    showMessage("Проект создан: " . $project->name);
                } else {
            ...
            case '4':
                ...
                if ($project) {
                    ...
                    echo "Задачи проекта (" . count($tasks) . "):\n";
            ...
            case '5':
                ...
                if ($projectService->deleteProject($projectId)) {
                    ...
                    showMessage("Ошибка удаления! Проверьте ID проекта и права доступа.");
                }
                break;
...
function showTaskMenu($taskService, $projectService, $userService) {
    while (true) {
        echo "\n=== УПРАВЛЕНИЕ ЗАДАЧАМИ ===\n";
        ...
        switch ($choice) {
            case '1':
                ...
                if ($task) {
                    showMessage("Задача создана: " . $task->title);
                } else {
                    showMessage("Ошибка создания задачи!");
                }
                break;
            ...
            case '3':
                ...
                if ($user && $taskService->assignTask($taskId, $user->id)) {
                    showMessage("Исполнитель назначен!");
                } else {
                    showMessage("Ошибка назначения исполнителя!");
                }
                break;
            ...
            case '4':
                ...
                if ($taskService->changeStatus($taskId, $status)) {
                    ...
                    showMessage("Ошибка изменения статуса!");
                }
                break;
...
function showUserMenu($userService) {
    while (true) {
